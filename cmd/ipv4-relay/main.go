// Command ipv4-relay is a lightweight IPv4 DHCPv4/ARP relay daemon.
package main
...
import (
	...
	"github.com/r2xpdhrlcmluzybjyw5hcnk/ipv4-relay/internal/relay"
)
...
const version = "v0.1-beta3"

func printUsage(app string, exitCode int) {
	fmt.Fprintf(os.Stderr, "== %s Usage ==\n"+
		"Lightweight IPv4 Relay Daemon (DHCPv4/ARP relay)\n\n"+
...
func main() {
	...
	if err := relay.SetLogSyslog(toSyslog, "ipv4-relay"); err != nil {
		fmt.Fprintf(os.Stderr, "openlog: %v\n", err)
	}

	// Raw sockets, netlink route operations and privileged UDP ports
	// (DHCPv4 67) are required. Running as root is the simplest way to
	...
	if !ipv4Enabled() {
		relay.Errorf("IPv4 is not enabled on this system")
		os.Exit(4)
	}
